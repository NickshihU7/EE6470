...
public:
	...
	addr_t twoD_FFT_start_addr = 0x80000000;
	addr_t twoD_FFT_size = 0x01000000;
	addr_t twoD_FFT_end_addr = twoD_FFT_start_addr + twoD_FFT_size - 1;
	...
	void parse(int argc, char **argv) override {
		...
		assert((mem_end_addr < clint_start_addr || mem_start_addr > twoD_FFT_end_addr) &&
...
int sc_main(int argc, char **argv) {
	...
	bus.ports[12] = new PortMapping(opt.twoD_FFT_start_addr, opt.twoD_FFT_end_addr);
	...
	bus.isocks[12].bind(two_D_FFT.tsock);
